...
parser.add_argument("--implementation", type=str, default='cuda_ste', help="implementation mode recommend cuda")
...
def main():
    ...
    #load model
    load_model_path = opt.outf.replace("rotation_","finetune_")
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in tqdm(enumerate(loader_train, 0), total=len(loader_train), desc="Training"):
            ...
            out_train = imgn_train + model.alpha*rotation_ensemble(imgn_train, model)
            ...
            if step % (opt.log_every*5) == 0:
                with torch.no_grad():
                    ...
                    for k in range(len(dataset_val)):
                        ...
                        out_val = sliding_window_processing(model, imgn_val, rotation=True, patch_size=192, overlap=20)
