...
parser.add_argument("--implementation", type=str, default='cuda_ste', help="Using ste mode for training")
...
def main():
    ...
    #load model
    load_model_path = opt.outf.replace("finetune_","")
    ...
    model.load_state_dict(torch.load(f"{load_model_path}/net_best.pth"))
    print(model)  
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in tqdm(enumerate(loader_train, 0), total=len(loader_train), desc="Training"):
            ...
            out_train = imgn_train + model.alpha * model(imgn_train)
            ...
            if step % (opt.log_every*10) == 0:
                ...
                with torch.no_grad():
                    ...
                    for k in range(len(dataset_val)):
                        ...
                        out_val = sliding_window_processing(model, imgn_val, patch_size=192, overlap=10)
                        psnr_val += batch_PSNR(out_val, img_val, 1.)

                        Img = utils.make_grid(img_val.data, nrow=1, normalize=True, scale_each=True)
                        Imgn = utils.make_grid(imgn_val.data, nrow=1, normalize=True, scale_each=True)
                        Irecon = utils.make_grid(out_val.data, nrow=1, normalize=True, scale_each=True)
